...
var result = ColorConverter.toCMYK(new RGB(fileArray[a][1], fileArray[a][2], fileArray[a][3]));

    r=parseFloat(fileArray[a][1]);
    g=parseFloat(fileArray[a][2]);
    bb=parseFloat(fileArray[a][3]);

              // c = parseFloat(result.c); // Second Column is Cyan
              // m = parseFloat(result.m); // Third Column is Magenta
              // y = parseFloat(result.y); // Forth Column is Yellow
              // k = parseFloat(result.k); // Fifth Column is Black
              //  c >= 0 && c <= 100 && m >= 0 && m <= 100 && y >= 0 && y <= 100 && k >= 0 && k <= 100) {
              //       var colArray = new Array(c, m, y, k);
               if (r >= 0 && r <= 255 && g >= 0 && g <= 255 && bb >= 0 && bb <= 255 ) {
                    var colArray = new Array(r,g,bb);
                    
                    var col = colors.add({space:ColorSpace.RGB, colorValue:colArray, name:b});
               } else {
